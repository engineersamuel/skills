---
name: finish
description: Use when and only when the user explicitly invokes $finish or /finish to commit current work, rebase onto origin/main, resolve conflicts, create a PR, enable auto-merge, and monitor it until merged.
...
4. Create the PR with `gh pr create`, then enable auto-merge with `gh pr merge --auto --rebase`.
